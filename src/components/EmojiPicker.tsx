...
export function EmojiPicker({ onSelect, onClose, position, currentEmoji }: EmojiPickerProps) {
  ...
  const getEmojisForCategory = (categoryName: string): EmojiItem[] => {
    if (categoryName === "常用") {
      return [...ALL_EMOJIS].sort((a, b) => {
        const countA = usageCount[a.emoji] || 0;
        const countB = usageCount[b.emoji] || 0;
        return countB - countA;
      });
    }
    return ALL_EMOJIS.filter((e) => e.category === categoryName);
  };

  const displayEmojis = searchQuery ? [] : getEmojisForCategory(CATEGORIES[activeCategory]);
  ...
  return (
    ...
    >
      <div className="flex border-b border-border px-2">
        {!searchOpen && (
          <div className="flex-1 flex gap-0.5 overflow-x-auto py-1 scrollbar-hide">
            {CATEGORIES.map((category, index) => (
              <button
                key={category}
                onClick={() => setActiveCategory(index)}
                className={`px-2 py-1.5 text-xs rounded-md transition-colors whitespace-nowrap flex items-center gap-1 ${
        ...
        <button
          ...
          className={`p-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-accent rounded-md transition-colors ml-1 ${
