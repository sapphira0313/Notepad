...
function CustomSlashMenu({ items, selectedIndex, onItemClick, onReorder }: {
  items: CustomSlashItem[];
  selectedIndex: number;
  onItemClick: (item: CustomSlashItem) => void;
  onReorder: (fromIndex: number, toIndex: number) => void;
}) {
  const [localItems, setLocalItems] = useState<CustomSlashItem[]>(() => {
    const saved = localStorage.getItem("slashMenuOrder");
    if (saved) {
      try {
        const order = JSON.parse(saved) as string[];
        const ordered: CustomSlashItem[] = [];
  ...
  const itemsRef = useRef<CustomSlashItem[]>(items);
  ...
  const handleMoveUp = (e: React.MouseEvent, index: number) => {
    ...
    localStorage.setItem("slashMenuOrder", JSON.stringify(newItems.map(item => item.id)));
  };
      ...
      {localItems.map((item, index) => {
        ...
        return (
          <div
            ...
            className={`w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition-all duration-150 group ${
              isSelected
          ...
          >
            ...
            >
              <span className="flex-shrink-0 w-7 h-7 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
                {item.icon}
              </span>
            ...
            {/* 右边：上下箭头，点击调整顺序 */}
            <div className={`flex flex-row gap-0.5 transition-opacity duration-150 ${isHovered ? "opacity-100" : "opacity-0"}`}>
              <button
                onMouseDown={(e: React.MouseEvent) => handleMoveUp(e, index)}
                disabled={index <= 0}
              ...
              <button
                onMouseDown={(e: React.MouseEvent) => handleMoveDown(e, index)}
...
export function BlockNoteEditor({
  ...
  const [sortedMenuItems, setSortedMenuItems] = useState<CustomSlashItem[]>(() => {
    const saved = localStorage.getItem("slashMenuOrder");
    if (saved) {
      try {
        const order = JSON.parse(saved) as string[];
        const ordered: CustomSlashItem[] = [];
        const remaining = new Map(SLASH_MENU_ITEMS.map(item => [item.id, item]));
        order.forEach(id => {
          const item = remaining.get(id);
          if (item) {
            ordered.push(item);
  ...
  const getItems = useCallback(async (query: string) => {
    const items = sortedMenuItems.map((item) => ({
      ...item,
  ...
  }, [handleItemClick, sortedMenuItems]);

  const suggestionMenuComponent = useCallback((props: any) => {
    const { items, selectedIndex, onItemClick } = props;
    return <CustomSlashMenu items={items} selectedIndex={selectedIndex ?? 0} onItemClick={onItemClick} onReorder={handleReorder} />;
  }, [handleReorder]);
